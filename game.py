...
from game_text import HOVER_HINTS, HUD_LINES, MESSAGES, NOTE_CLOSE_TEXT, NOTE_TEXTS, TUTORIAL_PAGES
...
class Game:
    ...
    def load_level(self, level_path):
        ...
        raw_objects = self.level_data["objects"]
        ...
        self.quantum_group_state = {
            group_id: {"was_observed": False}
            for group_id in self.quantum_groups
        }
    ...
    def rect_in_walkable(self, rect):
        ...
        for px, py in feet_points:
            ...
            if not self.is_walkable_cell(tx, ty):
                return False
        return True
    ...
    def handle_left_click(self, mouse_pos):
        ...
        if clicked_note:
            ...
            self.note_open_text = NOTE_TEXTS.get(clicked_note["id"], "...")
    ...
    def refresh_quantum_buttons(self):
        ...
        for button in self.quantum_buttons:
            ...
            button["active"] = button.get("pressed_once", False)
        ...
        for group_id, buttons in self.quantum_groups.items():
            ...
            for button in buttons:
                ...
                for cam in cameras:
                    if self.camera_sees_rect(cam, button["rect"]):
                        ...
                        visible_pairs.append((dist, button))
                        break

            observed_now = bool(visible_pairs)
            state = self.quantum_group_state.setdefault(group_id, {"was_observed": False})
            if observed_now and not state["was_observed"]:
                self.collapse_quantum_group(buttons)

            state["was_observed"] = observed_now

            if observed_now:
                visible_pairs.sort(key=lambda item: item[0])
                for _, button in visible_pairs:
                    if not button.get("collapsed_out", False) and not button.get("pressed_once", False):
                        button["observable"] = True

            for button in buttons:
                if button["removed"] or button.get("collapsed_out", False) or button.get("pressed_once", False):
                    continue
                if not button.get("observable", False):
                    continue
            ...
            resolved_count = sum(
                1
                for button in buttons
                if button.get("pressed_once", False) or button.get("collapsed_out", False)
            )
            if buttons and resolved_count >= len(buttons):
                target_id = buttons[0].get("target_id", "")
                if target_id:
                    self.open_door(target_id)

    def collapse_quantum_group(self, buttons):
        unresolved = [
            button for button in buttons
            if not button.get("pressed_once", False) and not button.get("collapsed_out", False) and not button.get("removed", False)
        ]
        if not unresolved:
            return
        ...
        if roll < 0.55:
            ...
            collapse_count = min(2, len(unresolved))
        else:
            collapse_count = len(unresolved)

        if collapse_count <= 0:
            return

        for button in random.sample(unresolved, collapse_count):
            button["collapsed_out"] = True

    # -------------------------------------------------------------------------
    # Camera visibility / ray logic
    # -------------------------------------------------------------------------
    def camera_sees_rect(self, cam, rect):
        target = rect.center
        ...
        return not self.line_blocked(origin, target)
    ...
    # Drawing
    # -------------------------------------------------------------------------
    def draw_tile_layer(self, screen, rows, skip_codes=None):
    ...
    def draw_note_overlay(self, screen):
        ...
        words = self.note_open_text.split()
        lines = []
        current = ""
        for word in words:
    ...
    def draw(self, screen, show_cursor=True):
        ...
        self.draw_tile_layer(world_surface, self.ground_rows, skip_codes=self.hidden_tile_codes)
        ...
        self.draw_tile_layer(world_surface, self.overlay_rows, skip_codes=self.hidden_tile_codes)
